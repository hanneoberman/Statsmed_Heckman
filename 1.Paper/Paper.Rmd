...
Thus, to estimate the parameters of the Heckman method, it is sufficient to specify the marginal distributions of the error terms, and link them with a suitable copula function. In our imputation method we estimate the Heckman model using the copula method, as in real life data generating mechanism may deviate a BVN distribution.
...
using the random effects 
 $b^O_i \sim N(0,\boldsymbol{\Psi^O})$ and $b^S_i\sim N(0,\boldsymbol{\Psi^S})$ with $\epsilon'^O_{i}\sim N(0,\sigma^O_{i}(X^O_i{X^O_i}^T))$ and $\epsilon'^S_{i}\sim N(0,\sigma^S_{i}(X^S_i{X^S_i}^T))$. From the meta model we obtain the estimates of parameters $\boldsymbol{\beta^S}$,$\boldsymbol{\beta^O}$, $\boldsymbol{\Psi^S}$,$\boldsymbol{\Psi^O}$.

Assuming that  $\sigma_i$ and $\rho_i$ are random variables comming from a populational distribution, they can express as:
$$ log(\sigma_i) \sim N( log(\sigma),\psi^{\sigma})$$
$$ tanh^{-1}(\rho_i)\sim N( tanh^{-1}(\rho),\psi^{\rho})$$ 
For each of them, we perform a univariate random effects meta_analysis. The model for $\sigma_i$ is given by the model:
