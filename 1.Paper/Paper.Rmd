...
#### Continuous incomplete variable
 ```{r rhocont,fig.align='center',fig.pos="!htb",fig.cap="Continuous incomplete variable by varing $\\rho$, the dashed line depicts the target performance criteria value",echo=FALSE,fig.keep="last",fig.height = 4, fig.width = 6}
plot_rho
 ```
...
In case of a bivariate missing variable, we also find (Figure \ref{fig:rhobin}) that the 2l.Heckman method provides unbiased results on coefficient parameters and random effects when the missing variable is binary. However, we observe more variability in the estimates of random effects, reflected on longer confidence intervals on the Bias and RMSE criteria.
 ...
 ```{r rhobin,fig.align='center',fig.pos="!htb",fig.cap="Binary incomplete variable by varing $\\rho$, the dashed line depicts the target performance criteria value",echo=FALSE,fig.keep="last",fig.height = 4, fig.width = 6}
plot_bin
```
...
```{r size,fig.align='center',fig.pos="!htb",fig.cap="Continuous incomplete variable under systematical missingness by varing: number of clusters (N); sample size per cluster ($n_i$), the dashed line depicts the true simulated parameter value",echo=FALSE,fig.keep="last",fig.height = 4, fig.width = 6}
plot_N
```
...
```{r dist,fig.align='center',fig.pos="!htb",fig.cap="Comparison of methods for continuous incomplete variable with deviations in distribution assumptions, where the dashed line depicts the true simulated parameter value",echo=FALSE,fig.keep="last",fig.height = 4, fig.width = 6}
plot_S
```
...
```{r descriptive,fig.align='center',cache = TRUE, results = "asis", table.pos="!htb", echo=FALSE}
...
```{r ERV,fig.align='center', cache = TRUE,results = "asis", table.pos="!htb", echo=FALSE}
if (!require("xtable")) install.packages("xtable")
xt <- xtable(tablexc,
...
```{r region,fig.align='center',fig.width = 5, fig.asp =.67, fig.pos="!htb", fig.cap="Estimates of malaria prevalence by Uganda subregion",echo=FALSE,fig.keep="last",cache=TRUE}
 plot_dist
```
...
```{r region_age,fig.align='center', fig.pos="!htb", fig.cap="Estimation of malaria prevalence by region and age",echo=FALSE,fig.keep="last",cache = TRUE}
plot_sdist_age
```
