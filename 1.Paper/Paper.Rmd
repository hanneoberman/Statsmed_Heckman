...
#### Continuous incomplete variable
 ```{r rhocont,fig.align='center',fig.pos="!htb",fig.cap="\\label{fig:rhocont} Continuous incomplete variable by varing $\\rho$, the dashed line depicts the target performance criteria value",echo=FALSE,fig.keep="last",fig.height = 4, fig.width = 6}
plot_rho
 ```
...
In case of a bivariate missing variable, we also find (Figure \ref{fig:bin}) that the 2l.Heckman method provides unbiased results on coefficient parameters and random effects when the missing variable is binary. However, we observe more variability in the estimates of random effects, reflected on longer confidence intervals on the Bias and RMSE criteria.

 ```{r rhobin,fig.align='center',fig.pos="!htb",fig.cap="\\label{fig:bin} Binary incomplete variable by varing $\\rho$, the dashed line depicts the target performance criteria value",echo=FALSE,fig.keep="last",fig.height = 4, fig.width = 6}
plot_bin
```
...
```{r size,fig.align='center',fig.pos="!htb",fig.cap="\\label{fig:size} Continuous incomplete variable under systematical missingness by varing: number of clusters (N); sample size per cluster ($n_i$), the dashed line depicts the true simulated parameter value",echo=FALSE,fig.keep="last",fig.height = 4, fig.width = 6}
plot_N
```
...
```{r dist,fig.align='center',fig.pos="!htb",fig.cap="\\label{fig:dist} Comparison of methods for continuous incomplete variable with deviations in distribution assumptions, where the dashed line depicts the true simulated parameter value",echo=FALSE,fig.keep="last",fig.height = 4, fig.width = 6}
plot_S
```
...
```{r region,fig.align='center',fig.width = 5, fig.asp =.67, fig.pos="!htb", fig.cap="\\label{fig:region} Estimates of malaria prevalence by Uganda subregion",echo=FALSE,fig.keep="last",cache = TRUE}
 plot_dist
```
...
```{r region_age,fig.align='center', fig.pos="!htb", fig.cap="\\label{fig:region_age} Estimation of malaria prevalence by region and age",echo=FALSE,fig.keep="last",cache = TRUE}
plot_sdist_age
```
