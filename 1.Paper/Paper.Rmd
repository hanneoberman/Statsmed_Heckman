...
Therefore, the aim of this paper is to develop a multilevel imputation method for incomplete continuous and binary variables that are MNAR. In section \ref{heckman} we provide an introduction to the Heckman model and its estimation, and we extended to a hierarchical setting. Then in the section \ref{methods} we define the main steps of the proposed imputation method. Posteriorly, in the section \ref{simulation} we provide the settings and results of a Montecarlo simulation study used to evaluate the performance of the proposed impuatation method. In section \ref{example} we illustrate the method using the survey information collected in different sub-districts in Uganda to estimate the prevalence of malaria in children. 
...
\end{align*}
We used these fixed coefficients $\beta_h^O=\{2,1,1\}$  and $\beta_h^S=\{0.1,1.5,-0.7,1.8\}$ in order to get around 40\% of sporadically missing values on the response $y_{ij}$ in the entire data set. Additionally, we made that the $y_{ij}^*$ observations were systematically missing in 20% of the clusters included in data set.
...
- \textbf{Binary response}: We evaluated the imputation method when missing variables is binary. Therefore we simulated  $y_{i}$ binary incomplete variables, the marginal set of parameters were fixed to  $B_h^O=\{0.5,1,1\}$ and  $B_h^S=\{0.1,1.5,-0.7,1.2\}$, keeping the other parameters similar to the ones used in the simulation of missing continuous variables. The observable binary variables was defined as:
\begin{align*}
    r_{i}&=I(r*_{i}>0)\\
...
#### Estimand
The estimads were the parameter coefficients of the outcome equation $\beta^O={\beta^O_0,\beta^O_1,\beta^O_2}$, with special emphasis on the treatment effect parameter $\beta^0_1$. We also report in the estimated variance of the random effects and residual errors $\sigma_{b0}^2$,$\sigma_{b1}^2$,$\sigma_{b2}^2$,$\sigma_{e}^2$.
...
We calculated the following evaluation criteria according to the formulas provided in Morris et al.(2019)[@morris2019]:

- \textbf{Bias:} Bias on the coefficient and random effect parameters.
- \textbf{EmpSE:} Empirical standard errors of the estimates on the coefficient and random effect parameters.
- \textbf{ModSE:} Monte Carlo standard errors on the coefficient  parameters.
- \textbf{MSE:} Mean squared error of the coefficient  and random effect parameters.
- \textbf{Coverage(\%):} Coverage of the 95\% confidence intervals for the coefficient  parameters.
In addition we reported the average time processing and the percentage of datasets where the imputation method converged, i.e., the imputation method generated an output. 
...
Processing times differ in the imputation methods evaluated, for example, for 10 clusters of 1000 units, the 1l. Heckman takes an average of 2 minutes, the 2l.MAR takes 10 minutes and the 2l. Heckman takes about 19 minutes. It should be noted that the time of the last two approaches depends strongly on the number of clusters included and the sample size in the data set. 

##### Results M(N)AR scenarios

 ```{r rhocont,fig.pos="h",fig.cap="\\label{fig:rhocont} Comparison of methods for continuous incomplete variable under systematical missingness by varing $\rho$, the dashed line depicts the true simulated parameter value",echo=FALSE,fig.keep="last"}
plot_rho
 ```
...
Overall, both Heckman-based imputation (1l. and 2l.) models provide less biased estimates of the coefficient parameters in MNAR scenarios, but the random effects estimates on the 1l.heckman are far away from the true values as no cluster information was considered at all.
 Regarding the coverage, the Heckman-IPD imputation method provides the best coverage values across all the $\rho$ scenarios with a coverage level close to the nominal 95\% interval. Even 2l.Heckman is not properly a randomization-valid (Rubin 1987b) method across all the $\rho$ values it leads to better results in terms of bias and coverage compared to the evalueated methods.  
...
The Heckman-IPD method, by incorporating individual and marginal level information, allows a bias-variance trade off, thus, as it was observed in Table \ref{tab:rhotable} the Heckman IPD separated resulted in better estimates in terms of RMSE than the other methods evaluated.
In particular, the method provides an advantage over the Heckman-single method in systematic missingness scenarios, since the latter does not allow to specify any cluster information implicitly, i.e. by adding a group variable in the imputation model. This can be seen in the estimates of the random effects parameters of the Heckman-single model, which are more biassed than those of the other methods in which cluster information was included.
Moreover, the method could be affected by the ratio of the pooled data (See Annex Figure\ref{fig:continuous3}). To demonstrate this, we increased the variance of the random effects in the simulated data, reducing the intra-cluster correlation (ICC) from 0.33 to 0.23. This resulted in less biassed estimates for both coefficient and random effects parameters, along with narrower confidence intervals.

##### Results sensitivity to number of clusters and sample size of clusters
We evaluated how robust our method was to variations in the sample size of the studies included in the IPD and also the number of clusters.
Our method allows for the inclusion of studies with different sample sizes, but to facilitate the evaluation of the method, in this simulation study we used 10 studies each with a fixed sample size of 1000 observations.

```{r rhosize,fig.pos="h",fig.cap="\\label{fig:size} Comparison of methods for continuous incomplete variable under systematical missingness by varing (number of studies- sample size per study), the dashed line depicts the true simulated parameter value",echo=FALSE,fig.keep="last"}
plot_N
```

When we reduced the sample size to 50 patients per study (left-most panel 10-50, Figure \ref{fig:rhosize}), we observed that the bias could be affected by some of the coefficient parameters, especially in MNAR scenarios. All simulated data were successfully imputed with the Heckman IPD methods but we observed that the precision of the parameters was drastically affected especially for the Heckman IPD full method, due to singularity fitting problems in the estimation of the mixed model, so we prefer not to show it in the graph.

On the other hand, by increasing the number of studies to 100 (rightmost panel 100-1000, Figure \ref{fig:rhosize}) we observe a higher precision of the estimates. There was an increase in the coverage of the estimates in the MAR scenario, i.e. rho=0, as the number of studies increases, which is in line with Audigier's results. However, in the case of MNAR, i.e. rho=0.6, we unfortunately observed a decrease in coverage, which could be explained by an increased complexity of the Heckman model.

Furthermore, we observed that for the full Heckman IPD method, the number of successfully imputed datasets was only 90% of the simulated datasets. This could be explained by the fact that increasing the number of studies included in the analysis might make it more difficult to estimate the marginal parameters when applying the meta-analysis model with random effects, especially when considering maximum likelihood or restrictive maximum likelihood as the estimation method. To ensure the estimation of the marginal parameters, our method evaluates the marginal parameters using the method of moments when it is not possible to estimate them with other methods chosen by the user.  We use the method of moments estimation as an alternative estimation method, as it was previously evaluated by Audigier, and was found to provide robust results when estimating the marginal parameters.
...
To investigate how the imputation models behaves in settings with departures of binormal distribution, we draw error terms from a bivariate skew student-t distribution using the same location parameter and covariance matrix of the normal distributed settings,  with 8 degrees of freedom and an $\alpha=\{-5,5\}$ parameter which regulates the the slant of the density. In addition we simulated a explicit missing process, where error terms of selection and outcome equation were normally independent distributed and the inclusion depends on the value of the missing variable, as follows:
$ry_i=0.1y_i+\epsilon^S_{i}$

These settings assure that the percentage of missingness is similar in all the evaluated scenarios around 60\%. We observed that for these simulated settings.

Figure \ref{fig:dist} depicts the results of this scenario. When we use the Heckman model in an explicit MNAR missing process, we observe that our model might not be completely suitable for this type of scenario, as we can observe that it affects the error variance which mainly affects the bias in the intersection coefficient parameter. and also its coverage. With respect to the t-distributed error scenario, apparently for the simulated measurements there is not much variation in the estimation of the coefficient parameters, however, we also observe that the estimation of the error variance was affected. In this scenario it is also seen that the coverage in the intercept parameter varies.  Similar variations occurred in the imputations performed with the simple Heckman imputation method, which is in line with the findings of @liu2020a.

##### Bivariate incomplete variable
We also evaluate the performance of the imputation methods for bivariate missing variables (Figure \ref{fig:bin}). We also find that the Heckman-IPD separate method provides unbiased results on coefficient parameters and random effects with better convergence compared to the other imputation method evaluated. However, we observe that the accuracy of the random parameter effects is sensitive to $rho$ the correlation between the outcome and the selection equation, resulting in more imprecise estimators as the $rho$ parameter increases.
...
To illustrate our method, following the article by @rugnao2019, we estimated the prevalence of parasitemia by subregion and by age after approximately 3 years of LLIN campaigns started.  We estimated parasitemia prevalence using 3 approaches: MCAR, MAR and MNAR.
